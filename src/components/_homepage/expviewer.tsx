...
export default function ExperienceArray(props: { exparr: Experience[] }) {
  const { exparr } = props;
      ...
      {activeItem === -1 ? (
        ...
        <div className={styles.expViewContainer}>
          <div className={styles.expContainer}>
            <div className={styles.expContainerLeft}>
              <span className={styles.expTitle}>
                {exparr[activeItem].title}
                <a
                  href={exparr[activeItem].link}
                  target="_blank"
                >{` @ ${exparr[activeItem].company}`}</a>{" "}
              </span>
              <span className={styles.expDuration}>
                {exparr[activeItem].range}
              </span>
            </div>
            <span
              className={styles.expMaxMin}
